...
namespace cohan{
    ...
    std::string Metrics::computeMetrics(int agent_id, double time_stamp){
        ...
        if(calculateSupriseMetrics()){
            cost_logs += result + " : H"+std::to_string(agent_id)+"_COST_REACT "+std::to_string(c_react)+"\n";
            cost_logs += result + " : H"+std::to_string(agent_id)+"_COST_SHOCK "+std::to_string(c_shock)+"\n";
            cost_logs += result + " : H"+std::to_string(agent_id)+"_COST_VISIBILITY "+std::to_string(c_visibility)+"\n";
        }

        if(calculateDangerMetrics()){
            cost_logs += result + " : H"+std::to_string(agent_id)+"_COST_FEAR "+std::to_string(c_fear)+"\n";
            cost_logs += result + " : H"+std::to_string(agent_id)+"_COST_PANIC "+std::to_string(c_panic)+"\n";            
        }
    ...
    bool Metrics::checkrobotSeen(){
            PoseSE2 human_pose_offset(human_pose_.x()-offset_map_x, human_pose_.y()-offset_map_y, human_pose_.theta()); 
            PoseSE2 robot_pose_offset(robot_pose_.x()-offset_map_x, robot_pose_.y()-offset_map_y, robot_pose_.theta());
