...
class MetricsData(object):
    ...
    def load_data(self, file_path, tags = []):
        ...
        if not tags:
                ...
                with open(file_path, "r") as file:
                    ...
                    for line in file:
                        ...
                        try:
                            ...
                            print("Format not found, skipping the line!")
                            continue

                        if "R" in line and not "COST" in line and not "H" in line:
                            split_data = line.split()
            ...
            toggle_start_end = False
            try:
                with open(file_path, "r") as file:
                    file.readline()
                    for line in file:
                        if start_tag in line and not toggle_start_end:
                            line = file.readline()
                            split_data = line.split()
                            time = float(split_data[0])
                            self.data['start_tags'].append(time)
                            toggle_start_end = True
                        if end_tag in line and start_tag != end_tag:
                            line = file.readline()
                            split_data = line.split()
                            time = float(split_data[0])
                            self.data['end_tags'].append(time)
                            toggle_start_end = False
                        elif end_tag in line and toggle_start_end:
                            line = file.readline()
                            split_data = line.split()
                            time = float(split_data[0])
                            self.data['end_tags'].append(time)
                        ...
                        if "R" in line and not "COST" in line and not "H" in line and not start_tag in line and not end_tag in line:
                            split_data = line.split()
                            time = float(split_data[0])
                            if not time in self.data['R']:
                                self.data['R'][time] = {}
                        ...
                        elif "H" in line and not start_tag in line and not end_tag in line:
                            split_data = line.split()
                            time = float(split_data[0])
                            h_id = split_data[2].split('_')[0]
                            if not h_id in self.data['H']:
    ...
    def get_costs(self, interval = [], type="time"):
        if type == "tag" or not interval:
            ...
            for i in range(len(self.data['start_tags'])):
                ...
                cost_set.append(self.calculate(intrv))
            return cost_set

        if type == "time":
            cost_set = []
            intrv = [float(interval[0]),float(interval[1])]
            cost_set.append(self.calculate(intrv))
            return cost_set
